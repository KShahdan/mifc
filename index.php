...
if(isset($_POST['action'])){
	...
	#echo "$action $userid $password";


	
	$query = "SELECT * FROM merchant_profile where username='$userid' and password = '$password' and status='1'";
	...
	if ($ctr > 0) {
		...
		if($datenotify >=1)
		{
		    header('Location: history_ex.php');
		}
		else{
...
if(isset($_POST['action1'])){
	...
	$query = "select mid from merchant_number where status='0' order by id asc limit 1";
	$res = mysql_query($query) or die ('Failed :'.mysql_error().'\n');
	if($rows=mysql_fetch_array($res))
	{
		$mid=$rows['mid'];
					...
					$statusfail= "<script type='text/javascript'>alert('Sorry your email address already exist!')</script>";


					$sql="insert into merchant_profile(id,username,password,email,first_name,last_name,merchant_name,merchant_id,acct_name,acct_num,bank_account,address,city,postal,greeting,logo,nric,status,dactivate,affialiate,dexpired)values ('','$email','$pwd','$email','$fname','$lname','$comp','$mid','','','','','','','','','$nric','1',now(),'$afia',NOW()+INTERVAL 14 DAY);";
					mysql_query($sql) or die ($statusfail);
                    ...
                    echo "<script type='text/javascript'>alert('Your account has been created. Please login with your email and password. Happy Selling!')</script>";
  ...
  <meta http-equiv="content-type" content="text/html; charset=UTF-8">
	...
	<style type="text/css">
		...
		body{
			...
			background-image: url('assets/img/mappingetauke.jpg');
		...
		h1,a,div,small,h6, .white{ color: #fff; font-family: 'Century Gothic';}
		.forms {
		border-radius: 20px;
			...
			<!-- dummy BOX ONE -->
				...
				<div class="col-xs-12 col-md-6" align="right">
					<form  name="contactForm" method="POST" action="index.php">
						<div class="form-group col-md-4">
							<input type="text" name="username" placeholder="Email" class="forms form-control " />
						</div>
						<div class=" form-group col-md-4">
							<input type="password" name="password" placeholder="Password" class="forms form-control"/>
						</div>
						<div class="form-group col-md-3">
	          				<button type="submit" class="btn btn-info forms btn-block" name="action">Log In</button>
	            ...
	            <div class="col-xs-12 col-md-7 extramag" align="right">
			            ...
			            <div class="col-xs-12 col-md-4">
			            	<p class="forgotten" align="center"><b><a href="forgot.php">Forgotten account?</a></b></p>
			            </div>
				</div>
			</div>
	</header>
		...
		<div class="row">
			<div class="col-xs-12 col-md-5 box2 pull-left">
				<div class="col-md-12">
					...
					<p class="white">Get a 14-day FREE trial</p>
				</div>
				...
				<form id="contactForm" class="form-horizontal" method="POST" action="index.php">
					...
					<div class="form-group">
						<div class="col-md-6">
							<input type="text" name="fname" placeholder="First Name" class="form-control forms"/>
						</div>
						<div class="col-md-6 laname">
							<input type="text" name="lname" placeholder="Last Name" class=" form-control forms lname"/>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-12">
							<input type="text" placeholder="Email address" name="email" class="form-control forms"/>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-12">
							<input type="password" name="pwd" placeholder="New password" class="form-control forms"/>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-12">
						<input type="text" name="companyname" placeholder="Company / Merchant Name" class="form-control forms"/>
						</div>
					</div>
					<div class="form-group">
						<!--<div class="col-md-12">
							<input type="text" name="nric"  placeholder="NRIC Number" class="form-control forms"/>
						</div>-->
						<div class="col-md-12">
						<p class="white"><small><h6>
						By clicking Create an account you agree to our <a href="#myModal" data-toggle="modal" data-target="#myModal">Terms and Conditions</a> and confirm that you have read our
						<a href="#myModal2" data-toggle="modal" data-target="#myModal2">Data Policy</a>. You may receive email notification from Botsphere
					...
					<div class="form-group">
						<div class="col-md-6">
							<button type="submit" class="btn btn-success btn-large col-md-12 forms" name="action1"><big>Create an account</big></button>
						</div>
					</div>
				</form>
				<!-- Modal popup for T&C -->
					...
					<div class="modal-dialog">
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								If you use our Services after the 14-days free trial, we collect information about the purchase or transaction. This includes your payment information, such as your credit or debit card number and other card information, and other account and authentication information, as well as billing, shipping and contact details.<hr/>
